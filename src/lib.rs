...
impl PowermaxB4800Status {
    pub fn from(bytes: &[u8]) -> Result<Self, Error> {
        ...
        let current = f32::from_be_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]); // unit: mA
        let voltage = f32::from_be_bytes([bytes[4], bytes[5], bytes[6], bytes[7]]); // unit: mV
        let temperature1 = bytes[8];
        let temperature2 = bytes[9];
        let temperature3 = bytes[10];
        let temperature4 = bytes[11];
        let remaining_energy = f32::from_be_bytes([bytes[12], bytes[13], bytes[14], bytes[15]]); // unit: mAh
